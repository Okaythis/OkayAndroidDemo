...
class MainActivity : AppCompatActivity() {
  ...
  override fun onCreate(savedInstanceState: Bundle?) {
    ...
    manualLinkButton.setOnClickListener {
      ...
      if (linkingCode.isEmpty()) {
        Toast.makeText(
          this,
          "Linking code can't be empty. Please enter linking code in the input field ",
          Toast.LENGTH_LONG
        ).show()
  ...
  private fun linkUser(linkingCode: String) {
    ...
    val linkingScenarioListener: LinkingScenarioListener = object : LinkingScenarioListener {
      ...
      override fun onLinkingFailed(var1: ApplicationState) {
        Toast.makeText(
          this@MainActivity,
          "Linking not Successful: linkingCode: ${linkingCodeEditText.text} errorCode: ${var1.code} ",
          Toast.LENGTH_LONG
        ).show()
      }
    }
  ...
  override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
    ...
    if (requestCode == PsaConstants.ACTIVITY_REQUEST_CODE_PSA_ENROLL) {
      if (resultCode == RESULT_OK) {
        ...
        Toast.makeText(
          this,
          "Error Retrieving intent after enrollment:- code: ${linkingCodeEditText.text} errorCode: $resultCode",
          Toast.LENGTH_SHORT
        ).show()
      }
    }
    ...
    if (requestCode == PsaConstants.ACTIVITY_REQUEST_CODE_PSA_AUTHORIZATION) {
      if (resultCode == RESULT_OK || resultCode == AuthorizationActivity.RESULT_OK_CONSUMED_PUSH) {
        Toast.makeText(this, "Authorization granted", Toast.LENGTH_SHORT).show()
      } else {
        val psaErrorData = PsaIntentUtils.psaErrorDataFromIntent(intent)
        val psaErrorMessage: String? =
          if (TextUtils.isEmpty(psaErrorData.message)) psaErrorData.status.toString() else psaErrorData.message
        Log.e(this.localClassName, psaErrorMessage!!.toString())
  ...
  private fun startServerAuthorization(userExternalId: String?) {
    transactionHandler.authorizeTransaction(userExternalId)
      .enqueue(object : Callback<AuthorizationResponse> {
        override fun onFailure(call: Call<AuthorizationResponse>, t: Throwable) {
          Toast.makeText(this@MainActivity, "Error making request to Server", Toast.LENGTH_LONG)
            .show()
        }
  ...
  private fun startServerLinking(userExternalId: String?) {
    transactionHandler.linkUser(userExternalId).enqueue(object : Callback<OkayLinking> {
      override fun onFailure(call: Call<OkayLinking>, t: Throwable) {
        Toast.makeText(
          this@MainActivity,
          "Error making request to Server ${t.localizedMessage}",
          Toast.LENGTH_LONG
        ).show()
        t.printStackTrace()
      }
  ...
  private fun startOTPTansaction(userExternalId: String?) {
    transactionHandler.authorizeOTPTransaction(userExternalId)
      .enqueue(object : Callback<AuthorizationResponse> {
        override fun onFailure(call: Call<AuthorizationResponse>, t: Throwable) {
          Toast.makeText(this@MainActivity, "Error making request to Server", Toast.LENGTH_LONG)
            .show()
